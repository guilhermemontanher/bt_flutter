...
class BluetoothPage2 extends StatefulWidget {
  ...
  const BluetoothPage2(this.deviceName, this.btInstance, this.btDevice);

  @override
  _BluetoothPage2State createState() => _BluetoothPage2State();
}
...
class _BluetoothPage2State extends State<BluetoothPage2> {
  ...
  _conect(BluetoothDevice device) {
    var a = FlutterBlue.instance.connect(device, autoConnect: false);

    a.listen((state) {
      print("MATHEUS : _conect: $state");
  ...
  _connect(BluetoothDevice device) {
    deviceConnection = btInstance
        .connect(device,
            timeout: const Duration(seconds: 4), autoConnect: false)
        .listen(
      (s) {
        print("teste $s");
...
//    });
    ...
    deviceConnection = device.onStateChanged().listen((s) {
      String estado;
